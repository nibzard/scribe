...
esp_err_t accel_gyro_bmi270_init(i2c_master_bus_handle_t bus_handle)
{
    ...
    bmi270.intf            = BMI2_I2C_INTF;  // Use I2C interface
...
void accel_gyro_bmi270_enable_sensor(void)
{
    ...
    if (rslt == BMI2_OK) {
        ...
        config[ACCEL].cfg.acc.odr = BMI2_ACC_ODR_200HZ;  // Accel ODR 200 Hz
        config[ACCEL].cfg.acc.range = BMI2_ACC_RANGE_4G;  // Accel range +/-4g
        config[GYRO].cfg.gyr.odr    = BMI2_GYR_ODR_200HZ;  // Gyro ODR 200 Hz
        config[GYRO].cfg.gyr.range = BMI2_GYR_RANGE_1000;  // Gyro range +/-1000 dps
