...
- In Typewriter/Centered mode the active line stays vertically centered; the view can scroll past the end to keep it centered, while Normal mode clamps to content bounds.
- Cursor movement, selection, undo/redo, and find are handled through keybindings.
- The editor viewport resizes to the current display resolution when the device rotation changes.
...
- Editor writing surface and viewport resize to the current display resolution on orientation change (portrait/landscape).
- Settings list keeps the selected row in view after list rebuilds (auto-sleep changes), so bottom items remain reachable in landscape.
...
- While TinyTTF is disabled, prefer ASCII punctuation in UI strings to avoid missing-glyph boxes.
